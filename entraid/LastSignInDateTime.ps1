...
$users = Get-MGUser  -Filter "signInActivity/lastSignInDateTime le $($date.ToUniversalTime().ToString("yyyy-MM-ddThh:mm:ssZ"))" -Select Id, userPrincipalName, mail, signInActivity
$selectedUsers = $users | SELECT ID, mail, userprincipalname, @{N="lastSignInDateTime"; e={$_.signInActivity.lastSignInDateTime}}
